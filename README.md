...
> 当前仓库的 Releases 仍在准备中。未经过 Apple Developer ID 签名和公证的自编译版本，在其他 Mac 上首次打开时可能需要右键点击 App 后选择“打开”。
...
按 EXIF 拍摄时间排序；无法读取 EXIF 时使用文件修改时间。输出格式为：
...
同名 RAW 的 `.xmp` 侧车会一起改名；已符合格式的照片会保留，编号会从当日已有最大编号继续。

### 🧹 RAW / JPG 配对清理

在**同一文件夹内**按同名主文件名配对，且不区分大小写：

```text
A001.JPG  ↔  a001.ARW
```

你可以选择清理“没有对应 RAW 的 JPG”，或“没有对应 JPG 的 RAW”。文件会进入废纸篓；恢复时优先使用隐藏安全恢复副本，必要时才请求 Finder 协助。

### ⭐ Adobe Bridge 星标与颜色同步

支持两个方向：`JPG → RAW` 与 `RAW → JPG`。可分别选择同步星标、颜色标签或两者。
